...
def create_mesh_ghost_icon(size=512, bg_color=None):
    """
    ...
    im = Image.new("RGBA", (canvas_size, canvas_size), bg_color if bg_color else (0, 0, 0, 0))
    ...
    # 1. Subtle ethereal glow background
    ...
    for r_step in range(int(glow_radius), 0, -int(8 * scale)):
        alpha = int(24 * (1.0 - r_step / glow_radius))
        glow_draw.ellipse(
    ...
    # 2. Ghost Tail flowing down from top/center
    ...
    for p1, p2 in edges:
        line_draw.line([p1, p2], fill=(157, 78, 221, 95), width=int(12 * scale))
    for p1, p2 in edges:
        line_draw.line([p1, p2], fill=(224, 170, 255, 235), width=int(4.5 * scale))
...
def generate_assets():
    ...
    # 3. Logo with Wordmark (Light text for dark backgrounds)
    wm_w, wm_h = 800, 200
    scale = 2
    ...
    print("✓ docs/assets/logo-wordmark.png")

    # 4. Logo with Wordmark - Dark Variant (for light backgrounds)
    wm_dark_im = Image.new("RGBA", (wm_w * scale, wm_h * scale), (0, 0, 0, 0))
    wm_dark_im.paste(icon_wm, (20 * scale, 20 * scale), icon_wm)
    wm_dark_draw = ImageDraw.Draw(wm_dark_im)

    wm_dark_draw.text((text_x, ghost_y), "GHOST", font=font_ghost, fill=(26, 16, 44, 255)) # deep void
    wm_dark_draw.text((text_x + 3 * scale, proto_y), "PROTOCOL", font=font_proto, fill=(107, 33, 168, 240)) # purple-800

    final_dark_wordmark = wm_dark_im.resize((wm_w, wm_h), Image.Resampling.LANCZOS)
    final_dark_wordmark.save("docs/assets/logo-wordmark-dark.png", "PNG")
    print("✓ docs/assets/logo-wordmark-dark.png")

    # 5. GitHub Social Preview (1280x640) - Fixed: Pre-composited icon on dark background
    sp_w, sp_h = 1280, 640
    sp_im = Image.new("RGBA", (sp_w, sp_h), (15, 15, 26, 255)) # #0F0F1A
    sp_draw = ImageDraw.Draw(sp_im)

    # Ambient center glow
    glow_cx, glow_cy = sp_w // 2, 205
    for r in range(320, 0, -8):
        alpha = int(28 * (1.0 - r / 320.0))
        sp_draw.ellipse(
            [glow_cx - r, glow_cy - r * 0.7, glow_cx + r, glow_cy + r * 0.7],
            fill=(123, 44, 191, alpha)
        )

    # Render icon directly with matching background to eliminate transparency checkerboard
    icon_size = 210
    icon_sp = create_mesh_ghost_icon(icon_size, bg_color=None)
    icon_card = Image.new("RGBA", (icon_size, icon_size), (15, 15, 26, 255))
    icon_card.alpha_composite(icon_sp)
    
    icon_x = (sp_w - icon_size) // 2
    icon_y = 90
    sp_im.paste(icon_card, (icon_x, icon_y))

    font_title = ImageFont.truetype(bold_font_path, 54)
    font_sub = ImageFont.truetype(regular_font_path, 26)
    font_tech = ImageFont.truetype(regular_font_path, 16)
    ...
    sp_draw.text(((sp_w - s_w) // 2, 405), sub_text, font=font_sub, fill=(199, 125, 255, 245))

    line_w = 120
    sp_draw.line([(sp_w - line_w) // 2, 455, (sp_w + line_w) // 2, 455], fill=(123, 44, 191, 150), width=2)

    tech_bbox = sp_draw.textbbox((0, 0), tech_text, font=font_tech)
    tech_w = tech_bbox[2] - tech_bbox[0]
    sp_draw.text(((sp_w - tech_w) // 2, 480), tech_text, font=font_tech, fill=(148, 163, 184, 230))

    f_bbox = sp_draw.textbbox((0, 0), footer_text, font=font_footer)
    f_w = f_bbox[2] - f_bbox[0]
    sp_draw.text(((sp_w - f_w) // 2, 570), footer_text, font=font_footer, fill=(100, 116, 139, 210))

    sp_im.save("docs/assets/social-preview.png", "PNG")
    print("✓ docs/assets/social-preview.png (alpha composited)")
